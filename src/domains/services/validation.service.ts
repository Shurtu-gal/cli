...
export class ValidationService extends BaseService {
  ...
  /**
   * Calculate document score based on various criteria
   */
  calculateDocumentScore(document: any): number {
    let scoreEvaluate = 0;
  ...
  private determineDiagnosticsStatus(
    ...
    const failSeverity = options.failSeverity ?? 'error';
   ...
   */
  private hasFailSeverity(diagnostics: Diagnostic[], failSeverity: SeverityKind): boolean {
    const diagnosticSeverity = getDiagnosticSeverity(failSeverity);
   ...
   */
  private formatStylish(diagnostics: Diagnostic[], options: { failSeverity: DiagnosticSeverity }): string {
